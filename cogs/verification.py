                ...
                if role_id_str:
                    ...
                    if verified_role:
                        try:
                            ...
                            print(f"[Verify] Forbidden: Could not assign role '{verified_role.name}' to {member}")
                    else:
                        print(f"[Verify] Warning: Configured role ID '{role_id_str}' not found in guild.")

            await interaction.followup.send(f"✅ **Verification Successful!** Your Discord account is now officially linked to the RSI Handle **{self.handle}**.\n\nYou may now remove the code from your RSI Short Bio.", ephemeral=True)
            self.stop()
        else:
            await interaction.followup.send(f"❌ **Verification Failed:** {message}\n\nPlease ensure you have correctly pasted ` {self.code} ` into your Short Bio and try again.", ephemeral=True)
...
class RSIVerification(commands.Cog):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(url) as response:
                    ...
                    return False, "The code was not found in your Short Bio. Sometimes RSI caches profiles—wait a minute and try again."
    ...
    async def verify(self, interaction: discord.Interaction, handle: str):
        ...
        if existing_handle:
            await interaction.response.send_message(f"You are already verified and linked to the RSI Handle **{existing_handle}**.", ephemeral=True)
            return
        ...
        embed = discord.Embed(
            ...
            description=f"To securely link your Discord account to the RSI Handle **{handle}**, please follow these steps:\n\n"
                        f"1. Go to your [RSI Account Profile](https://robertsspaceindustries.com/account/profile).\n"
                        f"2. Add the unique code below to the **END** of your **Short Bio**.\n"
                        f"3. Click **'Apply All Changes'** at the bottom of the RSI page.\n"
                        f"4. Click the **'Verify Now'** button below.",
    ...
    @app_commands.command(name="set_verified_role", description="Admin: Set the role granted upon successful RSI Verification.")
    ...
    async def set_verified_role(self, interaction: discord.Interaction, role: discord.Role):
        ...
        await interaction.response.send_message(f"✅ Verified role successfully set to {role.mention}.", ephemeral=True)

    @app_commands.command(name="grant_verified", description="Admin: Manually grant the verified role and RSI nickname to a member.")
    ...
    async def grant_verified(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if not rsi_handle:
            await interaction.response.send_message(
                f"❌ **{member.mention}** is not verified in the database. They must complete `/verify` first.",
                ephemeral=True
            )
            return
